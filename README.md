...
Записки от меня, чтобы не потерять

потенциально, надо делать тоже самое для hd видео - сейчас сетка берет максимум 320х240
какие данные нужны для обучения wav2lip 
гулим алгоритмы по превращению 2д в 3/4д (типа ок, в Юнити это делается из коробки, а если самим?)
https://www.youtube.com/watch?v=MtiUx_szKbI
там устанавливается targetImage, и если это изображение детектится, то запускается 3д модель
по видосу делается все изи, час работы для тех, кто в юнити шарит, вообще не сложно
нужна только 3д модель-ка
